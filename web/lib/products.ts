...
export type Product={
  ...
  problem_category?:string|null; problem_subcategory?:string|null;
  demand_allocation_score?:string|number|null; research_priority_score?:string|number|null; demand_winner_cap?:number|null;
  greek_gap_opportunity:string|null; greek_gap_confidence:string|number|null; sold_count:number|null; seller_source_id:string|null;
  product_identity:any; pain_feature_map:any[]; winning_factors:any[]; dealbreakers:any[]; pros:any[]; cons:any[];
  seller_quality:any; fulfillment_analysis:any; price_value_analysis:any; greek_fit_analysis:any; conversion_analysis:any;
  audience_personas:any[]; who_not_for:any[]; objection_handling:any[]; evidence_gaps:any[];
  intelligence_confidence:string|number|null; primary_keyword:string|null; secondary_keywords:any[]; long_tail_keywords:any[];
  faq_questions:any[]; title_options:any[]; meta_description_options:any[]; content_brief:any; seo_confidence:string|number|null;
  fact_count:number; media_count:number; review_count:number; spec_count:number; data_completeness:string;
};
...
export async function getProduct(id:string):Promise<Product|null>{
  ...
  if(!r.ok)return null;const j=await r.json();return j.data||null;
}
